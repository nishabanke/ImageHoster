...
public class ImageController {
    ...
    public String showImage(@PathVariable("title") String title, @PathVariable("imageId") Integer id, Model model) {
        ...
        System.out.println("----09e09we00000---------------");
        User loggedUser = (User)UserController.httpSession.getAttribute("loggeduser");
        ...
        List<Comment> filterdComments = new ArrayList();
        Iterator itr = allComments.iterator();
        while(itr.hasNext()) {
            Comment comment = (Comment) itr.next();
            if (image.getId().equals(comment.getImage().getId())) {
                filterdComments.add(comment);
            }
        }
        model.addAttribute("comments", filterdComments);
        return "images/image";
    }
    ...
    public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId) {
        ...
        List comments = commentService.getAllComments(imageService.getImage(imageId));
        if(comments != null && comments.size() !=0 ){
            commentService.deleteComments(comments);
        }
        System.out.println("----------------3333-----------delete");
